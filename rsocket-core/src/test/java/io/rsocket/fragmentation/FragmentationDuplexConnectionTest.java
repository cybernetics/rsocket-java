...
final class FragmentationDuplexConnectionTest {
  ...
  @DisplayName("fragments data")
  @Test
  void sendDataMultipleFragments() {
    ByteBuf encode =
      RequestResponseFrameFlyweight.encode(
        allocator, 1, false, Unpooled.EMPTY_BUFFER, Unpooled.wrappedBuffer(data));
    ...
    new FragmentationDuplexConnection(delegate, allocator, 64, false)
      .send(Flux.just(encode.retain(), encode.retain(), encode.retain(), encode.retain()));
  ...
  void sendEqualToMaxFragmentLength() {
    ...
    new FragmentationDuplexConnection(delegate, allocator, 4096, false)
      .send(Flux.just(encode.retain()));

    verify(delegate).send(publishers.capture());

    StepVerifier
      .create(Flux.from(publishers.getValue()))
      .assertNext(byteBuf -> {
        Assert.assertEquals(FrameType.REQUEST_RESPONSE, FrameHeaderFlyweight.frameType(byteBuf));
        Assert.assertFalse(FrameHeaderFlyweight.hasFollows(byteBuf));
      })
      .verifyComplete();
  }
